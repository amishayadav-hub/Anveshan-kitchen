...
export default function ReelPost({ post, active, liked, likeCount, onLike, onShare }: Props) {
  ...
  // Short, WhatsApp-friendly share: dish name + one line + a SINGLE recipe link.
  // Deliberately NO ingredients / steps / product links / long description, so
  // the message stays small and drives clicks to the page instead of dumping the
  // whole recipe (which read as spammy and buried the link).
  async function share() {
    onShare();
    track("share", { content_type: "community_post", item_id: post.id });
    // Canonical deep link to THIS post, UTM-tagged for attribution.
    const url = withShareUtm(`${SITE_URL}/recipes/real-peeps?post=${post.id}`, "real_peeps");
    const text = `🍽️ ${post.title}\n\nCheck out this delicious healthy recipe from Anveshan Kitchen.`;

    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        // Passing `url` separately lets WhatsApp/social render a clean link
        // preview from the page's Open Graph tags — no attached image needed.
        await navigator.share({ title: post.title, text, url });
        return; // native sheet handled it
      } catch (err) {
        if ((err as Error)?.name === "AbortError") return; // user dismissed the sheet
        // any other failure → copy fallback below
      }
    }
